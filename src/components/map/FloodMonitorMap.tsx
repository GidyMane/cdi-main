...
function pickBasin(
  districts: string,
  basins: FloodHoverBasin[],
): FloodHoverBasin | null {
  if (!basins.length) return null;
  const dn = districts.toLowerCase();
  return (
    basins.find(
      (b) =>
        b.name.toLowerCase().includes(dn) ||
        dn.includes(b.name.toLowerCase().split(" ")[0]),
      ...
      {hoveredDistrictName &&
        (() => {
          ...
          if (
            layerMode === "forecast" &&
            floodHoverData &&
            floodHoverData.basinStatus.length > 0
          ) {
            const basin = pickBasin(
              hoveredDistrictName,
              floodHoverData.basinStatus,
            );
            const trend = floodHoverData.basinTrend?.trend ?? "stable";
            const readings = floodHoverData.basinTrend?.readings ?? [];
            const currentLevel =
              floodHoverData.basinTrend?.current_level_m ?? basin?.level ?? 0;
            ...
            const popStr =
              basin && basin.population_at_risk >= 1_000_000
                ? `${(basin.population_at_risk / 1_000_000).toFixed(1)}M`
                : basin
                  ? `${(basin.population_at_risk / 1_000).toFixed(0)}K`
                  : "—";
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    <span
                      style={{ fontSize: 11, fontWeight: 700, color: headText }}
                    >
                ...
                >
                  ...
                  {basin && (
                    <div style={{ paddingBottom: 3, marginLeft: "auto" }}>
                      ...
                      >
                        {basin.discharge_rate.toFixed(0)}{" "}
                        <span style={{ fontSize: 9, color: dimText }}>
                          m³/s
                        </span>
                      </span>
                    </div>
                  ...
                  {topForecast && (
                    <div>
                      ...
                      >
                        ...
                        >
                          {topForecast.expected_level.toFixed(1)}m
                        </p>
                        <div
                          style={{
                            flex: 1,
                        ...
                        >
                          <div
                            style={{
                              height: "100%",
                              width: `${(topForecast.confidence ?? 0) * 100}%`,
                              backgroundColor:
                                topForecast.confidence >= 0.8
                                  ? "#22c55e"
                                  : topForecast.confidence >= 0.6
                                    ? "#eab308"
                                    : "#f97316",
                              borderRadius: 2,
                            }}
                        ...
                        <span style={{ fontSize: 8, color: dimText }}>
                          {Math.round((topForecast.confidence ?? 0) * 100)}%
                        </span>
                      </div>
                    </div>
                  )}
